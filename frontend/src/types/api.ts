...
export interface AnalysisEstimate { chunks: number; estimated_input_tokens: number; candidates: number }
...
export interface EditorElement { id: string; kind: ElementKind; x: number; y: number; width: number; height: number; z_index: number; visible: boolean; fit: MediaFit; opacity: number; border_width: number; border_color: string; radius: number; padding: number }
export interface CaptionStyle { enabled: boolean; font_family: string; font_size: number; color: string; weight: number; uppercase: boolean; outline_width: number; shadow: boolean; box_color: string | null; max_width: number; words_per_line: number; words_per_block: number; active_word_color: string | null }
...
export interface EditConfig { schema_version: 1; canvas_width: 1080; canvas_height: 1920; preset: LayoutPreset; elements: EditorElement[]; captions: CaptionStyle; banner: BannerStyle; background_color: string }
